<template>
  <div class="flex-1 bg-white shadow-inner border rounded-2xl p-4 flex flex-col">
    <div class="flex items-center justify-between mb-2">
      <div class="flex items-center gap-2">
        <ZoomControls @zoomIn="zoomIn" @zoomOut="zoomOut" @pan="panXY" @reset="resetView" />
      </div>
      <div class="flex items-center gap-2">
    ...
    <div class="flex-1 flex justify-center items-center rounded-xl overflow-hidden relative">
      <svg
        ref="svgRef"
        :width="svgWidth"
        :height="svgHeight"
 ...
 <GridRuler :width="svgWidth" :height="svgHeight" />
      ...
      <!-- floating UI info -->
      <div class="absolute left-4 top-4 bg-white/80 p-2 rounded shadow">
...
function renderAll() {
  ...
  props.tubes.forEach((t) => {
    ...
    c.addEventListener('click', () => {
      ...
      vp.querySelectorAll('circle[data-name]').forEach((z) => z.classList.remove('highlight'))
  ...
  txt.setAttribute('fill', '#334155')
  txt.setAttribute('font-size', '12')
  txt.textContent = `Reactor padding: ${props.config.padding} units`
  vp.appendChild(txt)
}
...
function onSearch(term: string) {
  ...
  if (!svg) return
  const vp = svg.querySelector('#viewport') as SVGGElement
  vp.querySelectorAll('circle[data-name]').forEach((c) => c.classList.remove('highlight'))
...
function downloadSvg() {
  const data = new Blob([svgRef.value?.outerHTML || ''], { type: 'image/svg+xml' })
