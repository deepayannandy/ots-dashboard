...
export function ensureLayers(
  vp: SVGGElement,
  tooltipEl: SVGTextElement | null = null,
) {
  ...
  tooltipEl = tip;
  // Ensure correct ordering of layers so tubes/icons stay above the boundary
  const order = [
    "boundary-layer",
    "highlight-layer",
    "tubes-layer",
...
export function computeAddRowForIndex(
  ...
  if (intendedIdx >= 0 && intendedIdx < rows.length)
    neighbor = rows[intendedIdx];
