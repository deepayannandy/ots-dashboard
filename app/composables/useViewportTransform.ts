...
export function useViewportTransform(
  defaults: ViewportOptions = { scale: 1, tx: 0, ty: 0, rotation: 0 },
) {
  ...
  function zoom(factor: number, pivotX = 0, pivotY = 0) {
    const prevScale = scale.value;
    const nextScale = prevScale * factor;

    // Preserve the pivot position while scaling.
    tx.value = (tx.value + pivotX) * (prevScale / nextScale) - pivotX;
    ty.value = (ty.value + pivotY) * (prevScale / nextScale) - pivotY;
  ...
  function rotate(degrees: number) {
    rotation.value = Math.max(-180, Math.min(180, rotation.value + degrees));
  }
