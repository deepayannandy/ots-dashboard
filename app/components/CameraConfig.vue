<template>
  ...
  <UModal
    v-model:open="localState.open"
    description="Configure Cameras"
  ...
  >
    <template #body>
      ...
      <UFieldGroup class="grid grid-cols-2 w-full">
        <div variant="outline" class="text-xl">Number of Cameras</div>
      ...
      <div v-if="localState.cameras && localState.cameras.length">
        <div
          v-for="(cameraId, index) in localState.cameras"
          :key="index"
          class="flex items-center gap-2"
        >
          <UFieldGroup class="grid grid-cols-2 w-full my-1">
            <div variant="outline" class="text-xl">Camera {{ index + 1 }}</div>
            <USelect
              v-model="localState.cameras[index]"
              :items="cameraItems"
...
const localState = reactive<Partial<TubeSheet> & { open: boolean }>({
  ...props.modelValue,
  open: props.open,
});
watch(
...
const handleSubmit = async () => {
  try {
    ...
    await useAxios().$patch(
      `/api/v2/tubeSheet/addCameraDetails/${localState._id}`,
      payload,
    );
    ...
    useToast().add({
      title: "Failed to save camera configuration",
      color: "error",
    });
  }
};
