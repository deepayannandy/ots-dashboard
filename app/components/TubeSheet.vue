<template>
  <!-- Modal -->
  <UModal v-model:open="open" description="Project Details">
    <template #body>
      <UForm :state="localState" class="grid grid-cols-1 gap-2">
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Project Start Date</div>
          <UInput
        ...
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Equipment ID</div>
        ...
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Equipment Type</div>
          <USelect
        ...
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Type of Phases</div>
          <USelect
        ...
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Client Name</div>
          <UInput v-model="localState.clientName" />
        </UFieldGroup>
        <UFieldGroup class="grid grid-cols-2 w-full place-items-start">
          <div variant="outline" class="text-xl">Client Address</div>
          <UTextarea v-model="localState.clientAddress" class="w-full" />
        </UFieldGroup>
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Material</div>
          <UInput v-model="localState.material" :disabled="isCloneing" />
        </UFieldGroup>
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Total No Of Tubes</div>
          <UInput
            v-model.number="localState.totalNoOfTubes"
            :disabled="isCloneing"
          />
        </UFieldGroup>
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Day Start Time</div>
          <UInput v-model="localState.dayStart" type="time" />
        </UFieldGroup>
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Day End Time</div>
          <UInput v-model="localState.dayEnd" type="time" />
        </UFieldGroup>
        <UFieldGroup class="grid grid-cols-2 w-full">
          <div variant="outline" class="text-xl">Timezone</div>
          <USelectMenu
  ...
  >
    <div
      class="flex flex-col items-center justify-center gap-3 text-neutral-400 dark:text-neutral-500"
    >
      <div
        class="p-3 rounded-full bg-neutral-100 dark:bg-neutral-800 elevation-1"
      >
  ...
  >
    ...
    <div class="space-y-1 text-xs text-neutral-600 dark:text-neutral-400">
      <p>
        ...
        {{ localState.equipmentId || "N/A" }}
      </p>
      <p>
        <span class="font-medium">WO:</span> {{ localState.workOrder || "N/A" }}
      </p>
      <p>
        <span class="font-medium">Type:</span>
        {{ getEquipmentTypeLabel(localState.type as string | undefined) }}
      </p>
      <p>
        <span class="font-medium">Site:</span> {{ localState.clientAddress }}
      </p>
      <p>
        <span class="font-medium">Material:</span>
        {{ localState.material || "N/A" }}
      </p>
      <p>
    ...
    <!-- Progress -->
    <div
      class="w-full h-1.5 bg-neutral-200 dark:bg-neutral-800 rounded-full overflow-hidden"
    >
      <div
    ...
    <UButton
      ...
      @click.stop="
        handleNextStep(localState.status, localState._id, localState.reactorId)
      "
    />
    <UButton
...
const props = defineProps<{
  modelValue: Partial<TubeSheet>;
  addNew?: boolean;
}>();
const emit = defineEmits([
  "update:modelValue",
  "saved",
  "update:openConfigCamera",
]);
...
const hasCamerasWithRtsp = computed(() => {
  ...
  return localState.cameras.some((cameraId) => {
    const camera = allCameras.value.find(
      (cam) => cam._id === cameraId || cam.name === cameraId,
    );
...
// Watch for prop changes and update state, resetting survey-related fields when switching sheets
watch(
  () => props.modelValue,
  (newVal) => {
    const oldSheetId = localState._id;
    const newSheetId = newVal._id;

    // If switching to a different sheet, reset survey-related status
    if (oldSheetId && newSheetId && oldSheetId !== newSheetId) {
      // Reset survey state when switching sheets to prevent old survey from resuming
      localState.surveyId = undefined;
      localState.isUnderSurvey = false;
    }

    // Update state properties from new value
...
const handleSubmit = () => {
  ...
  const payload: any = {
    ...localState,
    projectStartDate:
      localState.projectStartDate &&
      !isNaN(new Date(localState.projectStartDate).getTime())
        ? new Date(localState.projectStartDate).toISOString()
        : undefined,
    date:
      localState.date && !isNaN(new Date(localState.date).getTime())
...
const hasResumeJourney = computed(
  () => !!localState.isUnderSurvey && !!localState.surveyId,
);
...
const statusColor = (status?: string) => {
  switch (status) {
    case "TUBE_SHEET_CREATED":
      return "primary";
    case "CAMERA_CONFIGURED":
      return "secondary";
    case "REACTOR_CREATED":
      return "success";
...
const shouldShowEditReactor = (status?: string) => {
  return (
    status &&
    ["REACTOR_CREATED", "CAMERA_CALIBRATED", "IDLE", "UNDER_SURVEY"].includes(
      status,
    )
  );
};
...
const handleNextStep = (
  ...
  // Only allow resume journey if we have valid IDs and the surveyId belongs to this sheet
  if (hasResumeJourney.value && localState.surveyId) {
    if (sheetId && reactorId && sheetId === localState._id) {
      return navigateTo({
    ...
    // If sheet IDs don't match, don't resume - treat as new survey start
  }
  ...
  if (status && reactorId && sheetId)
    switch (status) {
      case "TUBE_SHEET_CREATED":
        return navigateTo(`/create-reactor/${sheetId}/${reactorId}`);
      case "CAMERA_CONFIGURED":
        return navigateTo(`/create-reactor/${sheetId}/${reactorId}`);
      case "REACTOR_CREATED":
        return navigateTo(`/survey-details/${sheetId}/${reactorId}`);
      case "CAMERA_CALIBRATED":
        return navigateTo(`/survey-details/${sheetId}/${reactorId}`);
      case "UNDER_SURVEY":
        return navigateTo(`/survey-details/${sheetId}/${reactorId}`);
    ...
    switch (status) {
      case "TUBE_SHEET_CREATED":
        return navigateTo(`/create-reactor/${sheetId}`);
      case "CAMERA_CONFIGURED":
        return navigateTo(`/create-reactor/${sheetId}`);
      case "REACTOR_CREATED":
        return navigateTo(`/create-reactor/${sheetId}`);
      case "CAMERA_CALIBRATED":
        return navigateTo(`/survey-details/${sheetId}`);
      case "UNDER_SURVEY":
        return navigateTo(`/survey-details/${sheetId}`);
      case "IDLE":
        return "Completed";
...
const phaseItems = computed(() => {
  const labelMap = Object.fromEntries(
    typeOfPhases.map((item) => [item.value, item.label]),
  );
