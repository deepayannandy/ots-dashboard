      ...
      <div class="space-y-6">
        <UPageGrid class="gap-5">
          ...
          >
            <div class="space-y-3">
              <span
                class="inline-flex size-14 items-center justify-center rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-500 elevation-1"
              >
                <UIcon name="i-lucide-plus" class="size-7" />
              </span>
              <div class="space-y-1">
                <p class="text-lg font-semibold">Add phase rule</p>
          ...
          <template v-if="isFetchingRules">
            <UPageCard v-for="s in 3" :key="`skeleton-${s}`" class="space-y-3">
          ...
          <template v-else-if="phaseRules.length">
            <UPageCard
              ...
              :spotlight-color="
                getColorSwatch(
                  rule?.configs?.baseColor || rule?.configs?.fsdEntry,
                ) || 'var(--color-primary)'
              "
            >
              ...
              <div
                v-if="rule.configs && Object.keys(rule.configs).length"
                class="space-y-2"
              >
                <div
              ...
              <p
                v-else
                class="rounded-lg border border-dashed border-neutral-200 px-3 py-2 text-sm text-muted dark:border-neutral-800"
              >
          ...
          >
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <p
                  class="text-xs font-semibold uppercase tracking-[0.3em] text-muted"
                >
                  {{
                    formState.phaseName === "COLOR_CAP_TRACKING"
                      ? "Cap colors"
                      : "Detection colors"
                  }}
                </p>
...
const getSchemaForPhase = (phase: string) =>
  phase === "COLOR_CAP_TRACKING" ? colorTrackingSchema : detectionSchema;
...
const isFormValid = computed(() => {
  ...
  return activeConfigFields.value.every(
    (field) => !!formState.configs[field.key],
  );
});
...
async function fetchPhaseRules() {
  ...
  try {
    const response = await axios.$get<PhaseRule[] | PhaseRuleApiResponse>(
      "/api/v2/phase/getAllPhases",
    );
