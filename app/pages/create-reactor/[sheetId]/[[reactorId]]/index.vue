      ...
      <UDashboardToolbar>
        ...
        <template #left>
          <USelectMenu
            ...
            placeholder="Select reactor layout"
            :items="layoutItems"
            value-key="value"
            class="min-w-80"
          ...
          >
            ...
            >
              <div class="space-y-2">
                ...
                <div
                  class="flex flex-wrap gap-1 max-w-auto max-h-50 overflow-scroll py-4"
                >
                  <span
                ...
                <div class="flex justify-end gap-2">
                  <UButton color="primary" @click="saveChanges"> Save </UButton>
                </div>
              </div>
            </UPageCard>
...
const layoutItems = ref<ReactorLayoutOption[]>([]);
const selectedLayoutId = ref<string | null>(null);
const layoutLoading = ref(false);

function formatLayoutLabel(item: ReactorLayoutItem) {
...
async function loadReactorLayouts() {
  layoutLoading.value = true;
  try {
    const response = await axios.$get<{
      Success: boolean;
      data: ReactorLayoutItem[];
    }>("https://apiots.dnyindia.in/api/v2/metadata/getAll");
    const layouts = response?.data || [];
    layoutItems.value = layouts.map((layout) => ({
    ...
    console.error("Failed to load reactor layouts", error);
    useToast().add({
      title: "Layouts Load Failed",
      description: "Could not fetch reactor layout metadata.",
      color: "error",
    });
  } finally {
...
async function loadSelectedLayoutJson(layoutId: string) {
  const layout = layoutItems.value.find((item) => item.value === layoutId);
  if (!layout?.fileUrl) return;

  layoutLoading.value = true;
  try {
    const fileUrl = layout.fileUrl.startsWith("http")
      ? layout.fileUrl
      : `https://apiots.dnyindia.in${layout.fileUrl}`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch layout JSON: ${response.status}`);
    }

    const jsonData = (await response.json()) as ReactorJson;
    generateReactorFromJson(jsonData);
    useToast().add({
      title: "Reactor layout loaded",
      description: `Loaded ${layout.label}`,
      color: "success",
    });
  } catch (error) {
    console.error("Failed to load reactor layout file", error);
    useToast().add({
      title: "Layout load failed",
      description: "Could not download reactor layout JSON.",
      color: "error",
    });
  } finally {
    layoutLoading.value = false;
...
watch(selectedLayoutId, async (layoutId) => {
  if (!layoutId) return;
  await loadSelectedLayoutJson(layoutId);
});
...
async function saveAndExportReport() {
  ...
  const existingReactorId =
    reactorId || (tubeSheetDetails.value?.reactorId as string | undefined);
